...
def detect_objects_by_vertical_blobs(color_map, grid_x, grid_y, hue_diff_thresh=2, min_vertical=3, slope_thresh=2):
    ...
    for j in range(len(grid_x) - 1):
        ...
        for i in range(len(grid_y) - 1):
            ...
            # Apply blur before taking mean to reduce small anomalies
            #blurred = cv2.GaussianBlur(cell[:, :, 0], (5, 5), 0)
            #mean_hue = np.mean(blurred)
            mean_hue = np.mean(cell[:, :, 0])
            #mean_hue = np.median(cell[:, :, 0])
...
def classify_clusters(clusters, depth_map, image_shape):
    """
    Classifies each blob into 'obstacle', 'wall', or 'dip' based on heuristics.
    """
    classified = []
    h, w = image_shape
...
class MJPEGStreamReaderThread(QThread):
    ...
    fps_updated = Signal(float)  # NEW: emit live FPS
...
class DepthGridViewer(QWidget):
    def __init__(self, stream_url):
        super().__init__()
        self.setWindowTitle("DriveCore - Grid + Depth Blob Classification")
    ...
    def update_frame(self):
        ...
        if self.depth_result is not None:
            ...
            # Create mask and extract contours
            mask = np.zeros((720, 1280), dtype=np.uint8)
            # FIXME: Disable for performance
            for (x1, x2, y1, y2) in alert_cells:
            ...
            blend, vanish_x, vanish_y = draw_perspective_grid(blend, highlight_zones=alert_cells)
            
            # FIXME: Disable for performance
            '''for (x1, x2, y1, y2) in alert_cells:
                cv2.rectangle(blend, (x1, y1), (x2, y2), (200, 200, 50), 1)  # light yellow'''
            ...
            for (x1, y1, x2, y2, label) in classified:
                ...
                cv2.putText(blend, f"{label} ({avg_depth:.2f}m)", (x1, y2 + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)

            output = blend

        else:
            # Just show video until depth is available
            output = frame

        self.video_label.setPixmap(QPixmap.fromImage(convert_to_qimage(output)))
