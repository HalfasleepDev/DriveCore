from PySide6.QtWidgets import QApplication, QLabel, QWidget, QVBoxLayout, QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QGraphicsTextItem
from PySide6.QtGui import QPixmap, QImage, QKeyEvent, QPainter, QFont
...
class DriveCoreUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("DriveCore Client (QGraphicsView)")
        self.setFocusPolicy(Qt.StrongFocus)

        # === Video Display ===
        self.scene = QGraphicsScene()
        self.view = QGraphicsView(self.scene)
        self.view.setViewport(QOpenGLWidget())  # Enable OpenGL acceleration
        ...
        self.overlay_text.setDefaultTextColor(Qt.green)
        self.overlay_text.setFont(QFont("Monospace", 12))
        self.overlay_text.setZValue(1)
    ...
    def send_ramping_command(self):
        if self.command:
            ...
            send_command(self.command, round(self.intensity, 2))
        else:
    ...
    def display_frame(self, data):
        ...
        if img is not None:
            ...
            # === Overlay text ===
            self.overlay_text.setPlainText(f"FPS: {self.fps:.1f} | Latency: {self.last_video_latency_ms} ms")
