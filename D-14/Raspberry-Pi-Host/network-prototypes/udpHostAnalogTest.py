...
current_esc_pw = NEUTRAL_DUTY_ESCt
current_servo_pw = NEUTRAL_SERVOt
...
def handle_control(sock, addr, payload):
    ...
    if last_timestamp is not None:
        ...
        if gap > TIMEOUT_MS:
            ...
            if current_esc_pw != NEUTRAL_DUTY_ESCt:
                current_esc_pw = NEUTRAL_DUTY_ESCt
                #pi.set_servo_pulsewidth(ESC_PIN, current_esc_pw)
                command = "NEUTRAL"
            if current_servo_pw != NEUTRAL_SERVOt:
                current_servo_pw = NEUTRAL_SERVOt
    ...
    if command == "UP":
        current_esc_pw = map_throttle(intensity, forward=True)
        #pi.set_servo_pulsewidth(ESC_PIN, current_esc_pw)

    elif command == "DOWN":
        current_esc_pw = map_throttle(intensity, forward=False)
        #pi.set_servo_pulsewidth(ESC_PIN, current_esc_pw)

    elif command == "LEFT":
        current_servo_pw = map_steering(intensity, left=True)
        #pi.set_servo_pulsewidth(SERVO_PIN, current_servo_pw)

    elif command == "RIGHT":
        current_servo_pw = map_steering(intensity, left=False)
        #pi.set_servo_pulsewidth(SERVO_PIN, current_servo_pw)
    ...
    print(f"Command: {command} | ESC: {current_esc_pw} | SERVO: {current_servo_pw} | From: {addr} | Time diff: {now - incoming_time}ms")

    ack = keyboard_command_ack_packet(command, current_esc_pw, current_servo_pw)
    sock.sendto(json.dumps(ack).encode(), addr)

# === Mapping Helpers ===
def map_throttle(intensity, forward=True):
    if forward:
        return int(NEUTRAL_DUTY_ESCt + (MAX_DUTY_ESCt - NEUTRAL_DUTY_ESCt) * intensity)
    else:
        return int(NEUTRAL_DUTY_ESCt - (NEUTRAL_DUTY_ESCt - MIN_DUTY_ESCt) * intensity)

def map_steering(intensity, left=True):
    if left:
        return int(NEUTRAL_SERVOt + (MAX_DUTY_SERVOt - NEUTRAL_SERVOt) * intensity)
    else:
        return int(NEUTRAL_SERVOt - (NEUTRAL_SERVOt - MIN_DUTY_SERVOt) * intensity)
